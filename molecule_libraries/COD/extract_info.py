...
# File Name,Metals,Component Count,Components With Metals,Polymolecular,Polymetallic,Organometallic,Organometallic3,Has Bonds,Hapticity,Delocalized,N_Atoms,Metal_Bonds,Overlapping Atoms
# cif/9/00/61/9006149.cif,"Cr(1), Fe(3), Mn(1), Ti(1)",15,04: Fe(1); 05: Mn(1); 06: Fe(1); 08: Fe(1); 10: Ti(1); 11: Cr(1),True,False,False,False,False,False,False,15.0,False,True
# cif/9/00/61/9006148.cif,"Cr(1), Fe(3), Mn(1), Ti(1)",15,04: Fe(1); 05: Mn(1); 06: Fe(1); 08: Fe(1); 10: Ti(1); 11: Cr(1),True,False,False,False,False,False,False,15.0,False,True
# cif/9/00/61/9006144.cif,"Cr(1), Fe(3), Mn(1), Ti(1)",15,04: Fe(1); 05: Mn(1); 06: Fe(1); 08: Fe(1); 10: Ti(1); 11: Cr(1),True,False,False,False,False,False,False,15.0,False,True

if __name__ == '__main__':
    # Load the CSV file into a DataFrame
    df = pd.read_csv('final_structures_refined.csv')
    # df = pd.read_csv('filtered_structures_mp_9.csv')
    ...
    print(f'First feature: {df.columns[0]} - File Name')
    ...
    print(f'Eighth feature: {df.columns[7]} - Explicit Hydrogens: True if the structure has explicit hydrogen atoms, False otherwise.')
    print(f'Ninth feature: {df.columns[8]} - Hapticity: True if the structure has hapto bonds, False otherwise.')
    print(f'Tenth feature: {df.columns[9]} - N_Atoms: Number of atoms in the structure.')
    print(f'Eleventh feature: {df.columns[10]} - Conectivity: Complete, Partial or None.')
    print(f'Twelfth feature: {df.columns[11]} - Metal Bonds: True if the structure has bonds involving metals, False otherwise.')
    ...
    # Get the number of entries with the 'Connectivity' complete or partial
    n_complete = len(df[df['Connectivity'] == 'Complete'])
    n_partial = len(df[df['Connectivity'] == 'Partial'])
    n_none = len(df[df['Connectivity'] == 'None'])
    print(f'The number of entries with complete connectivity is {n_complete}')
    print(f'The number of entries with partial connectivity is {n_partial}')
    print(f'The number of entries with no connectivity is {n_none}')
