...
def load_target_fragments_from_csv(entry_id, csv_path, threshold=0.999):
    df = pd.read_csv(csv_path)
    df = df[df["entry_id"] == entry_id].copy()

    df["fp"] = df["fp"].apply(lambda s: np.array([float(x) for x in s.split(',')]))
    df["formula"] = df["formula"].apply(eval)  # e.g., convert "('C', 'C4')" to tuple
...
def run_analysis(entry_id, population_file, idx, csv_path="all_fragments_data.csv", threshold=0.999):
    start = time.time()
    print(f"\n🔍 Target: {entry_id}")
    
    output_dir = "frag_comparison_results"
    ...
    for i, (fp_key, comp) in enumerate(comparisons.items(), start=1):
        ...
        with MoleculeWriter(output_path) as writer:
            ...
            for sim_score, sdf in comp["top_matches"]:
                ...
                # Use the custom write_entry method if available,
                # otherwise manually write the molecule and SD tag
