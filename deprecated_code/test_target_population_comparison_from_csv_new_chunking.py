...
def load_population_fragments_by_chunks(entry_ids, csv_path, chunk_size=100000):
    entry_ids = set(entry_ids)  # 💡 speeds up filtering
    chunks = pd.read_csv(csv_path, chunksize=chunk_size)
    for df in chunks:
        df = df[df["entry_id"].isin(entry_ids)].copy()
        if df.empty:
            continue
        df["fp"] = df["fp"].apply(lambda s: np.array([float(x) for x in s.strip("[] ").split(',')]))
...
def run_analysis(entry_id, population_file, idx, csv_path="all_fragments_data_0_5.csv", threshold=0.999):
    ...
    t2 = time.time()
    pop_fragments_gen = load_population_fragments_by_chunks(pop_ids, csv_path)
