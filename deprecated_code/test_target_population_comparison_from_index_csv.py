...
def load_population_subset_from_index(pop_ids, formulas, csv_path, index_path="fragment_index.csv", chunk_size=100_000):
    ...
    index_df = pd.read_csv(index_path, converters={"formula": ast.literal_eval})
    filtered = index_df[
        index_df["entry_id"].isin(pop_ids) &
        index_df["formula"].isin(formulas)
    ]
    ...
    # Group rows to read by chunk ID
    rows_by_chunk = defaultdict(list)
    for _, row in filtered.iterrows():
        rows_by_chunk[row["chunk_id"]].append(row["row_in_chunk"])

    # Read only necessary chunks and extract matching rows
    ...
    for chunk_id, chunk in enumerate(reader):
        ...
        try:
            ...
            print(f"⚠️ Some row indexes missing in chunk {chunk_id}. Skipping.")
            continue
