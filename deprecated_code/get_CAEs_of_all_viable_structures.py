...
def generate_fp_data(fragment, with_charges=False):
    arr_p = get_p_array_from_ccdcmol(fragment)
    ...
    fp_pq = None
    if with_charges:
        arr_pq = get_p_q_array_from_ccdcmol(fragment)
...
def main():
    ...
    duckdb_path = "all_fragments_3.duckdb"
    batch_size = 5000
    # with_charges_ids = ['ABAYAF', 'RULJAM']
    fragment_size_counter = defaultdict(int)
    ...
    for batch in chunk_list(entries, batch_size):
        ...
        with Pool(n_proc) as pool:
            for frag_list in pool.imap_unordered(process_population_entry, batch):
                results.extend(frag_list)

        if not results:
            print(f"✅ No fragments in batch {batch_number}; skipping.")
            continue

        df = pd.DataFrame(results)
        
        # 🔢 Update fragment size distribution
        for size, count in df["n_atoms"].value_counts().items():
            fragment_size_counter[size] += count
        ...
        con.register("df", df)
        con.execute("""
            INSERT INTO fragments AS f
            SELECT * FROM df
            ON CONFLICT (entry_id, frag_idx) DO UPDATE SET
                fp_p = COALESCE(f.fp_p, EXCLUDED.fp_p),
                fp_pq = COALESCE(f.fp_pq, EXCLUDED.fp_pq),
                sdf = EXCLUDED.sdf,
    ...
    print(f"\n📊 Total fragments processed: {total_fragments_written}")
    print("📈 Fragment size distribution:")
    for size in sorted(fragment_size_counter):
        count = fragment_size_counter[size]
        pct = 100 * count / total_fragments_written
        print(f"  Size {size}: {count} fragments ({pct:.2f}%)")
        
    elapsed = time.time() - start
    print(f"\n✅ All data inserted into '{duckdb_path}' table 'fragments'.")
