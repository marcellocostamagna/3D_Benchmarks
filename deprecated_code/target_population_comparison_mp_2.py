...
def process_molecule_parallel(entry_id):
    """Parallel function for processing a molecule: returns only fingerprints."""
    try:
...
for i in range(0, len(population_entry_ids), batch_size):
    ...
    batch_fps = find_unique_fragments_optimized(batch_fps)

    # Compare to target fragments
    _, n_matches = compare_target_to_population(target_fps, batch_fps)

    if n_matches == len(target_fps):
        print("\n✅ Found all unique target fragments before finishing population analysis. Stopping early.")
        break

    # Convert fingerprints to hashable format before adding to the set
    found_fragments.update(tuple(fp) for fp in batch_fps)
