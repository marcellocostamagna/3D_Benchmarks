...
def _parse_fp_chunk(rows, with_sdf: bool):
    ...
    with_sdf  : bool
        True ⟹ each row is (cae_id, fp_p[_pq], sdf)
        False ⟹ each row is (cae_id, fp_p[_pq])
    """
    parsed = []
...
def compare_formula_streaming(
    ...
    need_sdf = target_caes[0]["n_atoms"] == 2          # bi‑atomic only
    ...
    # Final distorted match handling
    for tdata in target_caes:
        tkey = fingerprint_key(tdata["fp"])
...
def run_analysis(entry_id, population_file, idx,
    ...
    start = time.time()
    print(f"\n🔍 Target: {entry_id}")
    output_dir = "cae_comparison_results_duckdb"
    os.makedirs(output_dir, exist_ok=True)

    # --- 1) Generate target CAEs ---
    t0 = time.time()
...
if __name__ == "__main__":
    ...
    for i, target in enumerate(targets, start=1):
        run_analysis(
            entry_id=target,
            population_file=f"Starting_populations_0_5/{i}_{target}_init_pop.txt",
            idx=i,
            db_path="all_caes.duckdb",
            target_threshold=0.999,
