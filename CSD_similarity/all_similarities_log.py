...
# Setup logging
logging.basicConfig(filename='similarity_calculation.log', level=logging.INFO, 
...
def calculate_similarity_pybel(data):
    ...
    for smiles_str in entry_smiles:
        try:
            ...
            logging.error(f"Error reading smiles string in entry: {entry}. Error: {e}")
            break 
...
if __name__ == '__main__':
    ...
    for target_fp_3d, target_fp_4d, target_smiles, target_identifier, result_file_3d, result_file_4d, result_file_pybel, result_file_csd, label in zip(target_fps_3d, target_fps_4d, target_smiles_list, target_identifiers, results_files_3d, results_files_4d, results_files_pybel, results_files_csd, target_labels):
        ...
        start_3d = time.time()
        logging.info(f"Calculating 3D similarity for {len(entries)} entries with {cpu_count()} CPUs for target {label}...")
        ...
        logging.info(f'Saving 3D results to {result_file_3d}...')
        ...
        logging.info(f'Time taken for 3D similarity: {time_3d/60:.2f} minutes\n')

        # 4D Similarity calculations
        start_4d = time.time()
        logging.info(f"Calculating 4D similarity for {len(entries)} entries with {cpu_count()} CPUs for target {label}...")
        with Pool(processes=cpu_count(), initializer=init_process, initargs=(target_fp_3d, target_fp_4d, fingerprint_dict_3d, fingerprint_dict_4d)) as pool:
            results = pool.map(calculate_similarity, entries)
        logging.info(f'Saving 4D results to {result_file_4d}...')
        ...
        logging.info(f'Time taken for 4D similarity: {time_4d/60:.2f} minutes\n')

        # Pybel Similarity calculations
        start_pybel = time.time()
        logging.info(f"Calculating pybel similarity for {len(entries)} entries with {cpu_count()} CPUs for target {label}...")
        ...
        logging.info(f'Saving pybel similarity results to {result_file_pybel}...')
        ...
        logging.info(f'Time taken for pybel similarity: {time_pybel/60:.2f} minutes\n')

        # CSD Similarity calculations
        start_csd = time.time()
        logging.info(f"Calculating CSD similarity for {len(entries)} entries with {cpu_count()} CPUs for target {label}...")
        with Pool(processes=cpu_count()) as pool:
            data = [(entry, target_identifier) for entry in entries]
            results = pool.map(calculate_similarity_csd, data)
        logging.info(f'Saving CSD similarity results to {result_file_csd}...')
        ...
        logging.info(f'Time taken for CSD similarity: {time_csd/60:.2f} minutes\n')

        target_time_elapsed = time.time() - target_start
        logging.info(f'Total time taken for target {label}: {target_time_elapsed/60:.2f} minutes\n')

    overall_time_elapsed = time.time() - overall_start
    logging.info(f'Total time elapsed: {overall_time_elapsed/60:.2f} minutes')
