...
# --------------- SET UP LOGGING --------------- #
log_filename = "cae_extraction.log"
logging.basicConfig(
...
def process_population_entry(entry_id, with_charges_ids=None):
    try:
        ...
        logging.error(f"Error processing {entry_id}: {e}")
        return []

def extract_entries_from_csv(csv_path):
    df = pd.read_csv(csv_path)
    identifiers = df["Identifier"].dropna().unique().tolist()
    logging.info(f"Total unique entries: {len(identifiers)}")
    return identifiers
...
def main():
    start = time.time()
    input_csv = "../Filtering/viable_structures.csv"
    ...
    logging.info(f"Using {n_proc} processes")
    ...
    for batch in chunk_list(entries, batch_size):
        batch_number += 1
        logging.info(f"Processing batch {batch_number} / {len(entries)//batch_size + 1}")
        ...
        if not results:
            logging.info(f"No CAEs in batch {batch_number}; skipping.")
            continue
        ...
        required_cols = [
            "entry_id", "cae_idx", "sdf", "fp_p", "fp_pq",
        ...
        if df.empty:
            logging.info(f"DataFrame for batch {batch_number} is empty; skipping insert.")
            continue
        ...
        logging.info(f"Saved batch {batch_number} ({batch_cae_count} CAEs)")
